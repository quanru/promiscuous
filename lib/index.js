...
(function (func, obj) {
  ...
  function is(type, item) {
    return (typeof item === 'undefined' ? 'undefined' : _typeof(item))[0] == type;
  }
  ...
  function Promise(callback, handler) {
    ...
    handler = function pendingHandler(resolved, rejected, value, queue, then, i) {
      ...
      // The value is not a promise; handle resolve/reject
          ...
          handler = function handler(Resolved, Rejected, isSpread) {
            // If the Resolved or Rejected parameter is not a function,
            // return the original promise (now stored in the `callback` variable)
            if (!is(func, Resolved = rejected ? Resolved : Rejected)) return callback;
            // Otherwise, return a finalized promise, transforming the value with the function
            return Promise(function (resolve, reject) {
              finalize(this, resolve, reject, value, Resolved, isSpread);
            });
          };
          ...
          while (i < queue.length) {
            ...
            else finalize(then.p, then.r, then.j, value, resolved, then.isSpread);
          }
        }
    };
    ...
    callback.call(callback = { then: function then(resolved, rejected) {
        return handler(resolved, rejected, false);
      },
      spread: function spread(resolved, rejected) {
        return handler(resolved, rejected, true);
      },
  ...
  function finalize(promise, resolve, reject, value, transform, isSpread) {
    ...
    mySetImmediate(function () {
      try {
        ...
        if (isSpread) value = Array.isArray(value) ? transform.apply(undefined, _toConsumableArray(value)) : transform(value);else value = transform(value);
